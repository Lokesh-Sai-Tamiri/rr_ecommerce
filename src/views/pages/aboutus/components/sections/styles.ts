/**
 * @fileoverview Centralized styles for About Us section components using landing/utils
 */
...
/**
 * Custom hook for about us component styles using centralized utilities
 */
export const useAboutUsStyles = () => {
  ...
  const styles = useMemo(() => {
    ...
    // Fix: Match the expected structure for AboutCard component
    const fontSizes = {
      title: {
      ...
      // Fix: Add the missing icon property
    ...
    const getCardStyles = (index: number): SxProps<Theme> => {
      ...
      return {
        ...
        maxWidth: screen.isDesktop ? '600px' : '100%',
        margin: '0 auto',
    ...
    // Grid styles using centralized responsive logic - 2 columns on desktop, 1 on mobile
    const gridStyles: SxProps<Theme> = {
      ...
      alignItems: 'stretch',
      mx: 'auto',
      maxWidth: screen.isDesktop ? '1400px' : '100%'
    };
 ...
 */
export const getAboutUsSpacing = (screen: ReturnType<typeof useScreenDetection>, theme: Theme) => {
  return {
 ...
 */
export const getAboutUsFontSizes = (screen: ReturnType<typeof useScreenDetection>) => {
  const mobileSize = getMobileSizeWithFallback(screen);
  const mobileScaling = getMobileScaling(mobileSize);
 ...
 */
export const getAboutUsAnimations = (screen: ReturnType<typeof useScreenDetection>) => {
  return {
