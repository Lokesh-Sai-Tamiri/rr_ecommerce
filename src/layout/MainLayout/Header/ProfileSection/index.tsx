...
export default function ProfileSection() {
  ...
  const { user: userContextUser, logout: userLogout } = useUser();
  const [open, setOpen] = useState(false);

  // Use UserContext user data if available, fallback to auth user
  const displayUser = userContextUser || authUser;
  const displayName = userContextUser
    ? `${userContextUser.firstName} ${userContextUser.lastName}`.trim()
    : authUser?.name || 'User';
  ...
  const handleLogout = async () => {
    try {
      ...
      // Now logout from UserContext after Clerk is signed out
      if (userContextUser) {
        console.log("🔓 Logging out from UserContext");
        userLogout();
      }

      // Another small delay to ensure all cleanups complete
      await new Promise(resolve => setTimeout(resolve, 100));

      console.log("✅ Logout completed - all systems should be cleared");
