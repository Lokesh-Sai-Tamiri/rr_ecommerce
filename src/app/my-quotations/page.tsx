/**
 * @fileoverview My Quotations page - Main page component
 */
...
export default function MyQuotationsPage() {
  const [selectedTab, setSelectedTab] = useState("my-quotations");
  const [quotationSubTab, setQuotationSubTab] = useState("requested");
  const screen = useScreenDetection();
  const { user } = useUser();
  const searchParams = useSearchParams();

  // Handle URL query parameter to set initial tab
  useEffect(() => {
  ...
  // Fetch user cart items from database (DYNAMIC DATA)
  const {
    cartItems,
    requestedCartItems,
    validCartItems,
    ...
    autoFetch: true,
    // Fetch all statuses; the hook will gather requested, valid, expired
  });
            ...
            >
              {selectedTab === "my-quotations" && (
                <QuotationsContent
                  quotations={[]}
                  cartItems={
                    quotationSubTab === "requested"
                      ? requestedCartItems
                      : quotationSubTab === "valid"
                        ? validCartItems
                        : expiredCartItems
                  }
                  ...
                  cartError={cartError}
                  onCartRefresh={refreshCartItems}
                  isRealData={!!user?.id}
                />
              )}
