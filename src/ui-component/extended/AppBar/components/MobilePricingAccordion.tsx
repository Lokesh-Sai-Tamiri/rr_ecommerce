/**
 * @fileoverview Mobile pricing accordion component for drawer navigation
 */
...
export default function MobilePricingAccordion({ dropdownItems, onItemClick }: MobilePricingAccordionProps) {
  ...
  const handlePricingClick = (href: string) => {
  ...
  const isPricingActive = dropdownItems.some((item) => isActiveItem(item.href));
  ...
  return (
    ...
    >
      <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls="pricing-panel-content" id="pricing-panel-header">
        <Typography
          variant="h6"
          sx={{
            fontSize: '1rem',
        ...
        >
          <FormattedMessage id="navigation.mobile.pricing" defaultMessage="Pricing" />
        </Typography>
      </AccordionSummary>
      <AccordionDetails>
          ...
          {dropdownItems.map((item, index) => {
            ...
            return (
              <ListItem key={index} disablePadding>
                <ListItemButton
                  onClick={() => handlePricingClick(item.href)}
                  sx={{
                    borderRadius: 2,
                    mb: 1,
                    py: 1,
