...
export default function componentStyleOverrides(theme: Theme, borderRadius: number, outlinedFilled: boolean) {
  ...
  // Get base overrides
  const baseOverrides = componentsOverrides(theme);

  // Extract MuiButton from base overrides to avoid conflicts
  const { ...otherOverrides } = baseOverrides;

  return {
    // Spread other overrides (excluding MuiButton)
    ...otherOverrides,
    // Define MuiButton with both base variants and our styleOverrides
    MuiButton: {
      styleOverrides: {
        root: {
          fontWeight: 500,
